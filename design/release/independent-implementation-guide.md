...
> Guide for filling the independent-maintenance evidence template in [`design/release/rc-validation.md`](rc-validation.md). This guide explains what counts as independent maintenance and how to complete each field. It does **not** itself claim that any implementation is independently maintained; that status remains `PENDING` until evidence is recorded and reviewed.
...
The evidence you produce must map one-to-one onto the required matrix rows in `design/release/rc-compatibility-matrix.md`:
...
- Record the topology: `harmovela.topology.hub-spoke.v1` (version `v1`), declared in `design/protocol/profiles.md`.
